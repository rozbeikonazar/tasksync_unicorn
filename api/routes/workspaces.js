...
router.get("/", checkAuth, async function (req, res) {
  ...
  try {
    ...
    const workspaceInvitations = await UserWorkspaceInvintation.find({
      user_id: userID,
    }).populate("workspace_id");
    const workspacesInvited = workspaceInvitations.map(
      (invitation) => invitation.workspace_id
    );
...
router.get(
  ...
  async function (req, res) {
    ...
    try {
      ...
      const invitedUsers = await UserWorkspaceInvintation.find({
...
router.post(
  "/:workspaceID/generate_invintation",
  checkAuth,
  isWorkspaceCreator,
  async function (req, res) {
...
router.post("/:uuid/join", checkAuth, async function (req, res) {
  ...
  const userID = req.user._id;
  try {
    const invitation = await Invintation.findOne({
      url: `${HOST}${WORKSPACE_PREFIX}${req.params.uuid}/join`,
    });
    if (!invitation) {
    ...
    if (workspace) {
      return res
        .status(400)
        .json({ error: "User is the creator of the workspace" });
    }
  ...
  try {
    ...
    if (workspace) {
      return res.status(400).json({
        error:
          "User is the creator of the workspace. You can delete workspace, but not quit it",
      });
    }
    const result = await UserWorkspaceInvintation.deleteOne({
      user_id: userID,
      workspace_id: workspaceID,
    });
    ...
    if (!result) {
      return res.status(400).json({
        error:
          "Could not quit from workspace or you are not a member of this workspace",
      });
    }
...
router.delete(
  ...
  async function (req, res) {
    ...
    try {
      ...
      if (!result) {
        return res.status(400).json({
          error: "Could not kick a user or he is not a member of workspace",
        });
      }
      return res
        .status(200)
        .json({ message: "Sucessfully kicked user from workspace" });
    } catch (err) {
...
router.post(
  "/:workspaceID/:taskID/add_comment",
  checkAuth,
  checkWorkspaceAccess,
